...
namespace HOME.Game {
    ...
    public class CameraManager : MonoBehaviour {
        ...
        private Camera mainCamera = null; //the main camera
        [SerializeField] public Camera miniMapCamera = null; //the main camera

        [SerializeField, Header("Sight Fader Player Ship"), Tooltip("The main camera in the scene.")]
        public float DistanceToPlayer = 200.0f;
        public Material TransparentMaterial = null;
        public Shader TransparentShader = null;
        public float FadeInTimeout = 0.6f;
        public float FadeOutTimeout = 0.2f;
        public float TargetTransparency = 0.1f;
        ...
        private float initialHeight = 50.0f; //the initial height that the camera starts with
        ...
        private void Start() {
            ...
            foreach (var p in gameMgr.Factions) { //set cam to player position
                ...
                var pos = p.playerLocation.position;
                pos.y = initialHeight;
                pos.x += 10f;
                pos.z -= 45f;
        ...
        private void Update() {
            //check for player ship near camera pos
            SightFaderShip();
        ...
        private void SightFaderShip() {
            RaycastHit[] hits;
            hits = Physics.RaycastAll(transform.position, transform.forward, DistanceToPlayer, layer_mask);
            foreach (RaycastHit hit in hits) {
                ...
                SightMakeTransparent AT = R.GetComponent<SightMakeTransparent>();
                if (AT == null) {// if no script is attached, attach one
                    AT = R.gameObject.AddComponent<SightMakeTransparent>();
        ...
        private void Pan() {
            ...
            {
                var temp = followTarget.position.z - 40f;
                LookAt(new Vector3(followTarget.position.x, followTarget.position.y, temp), smoothFollow, smoothFollowFactor);
            ...
            if (panLimit.enabled)
                mainCamera.transform.position = new Vector3(
                    Mathf.Clamp(mainCamera.transform.position.x, panLimit.minPosition.x, panLimit.maxPosition.x),
                    mainCamera.transform.position.y,
                    Mathf.Clamp(mainCamera.transform.position.z, panLimit.minPosition.y, panLimit.maxPosition.y));
        ...
        public void LookAt(Vector3 targetPosition, bool smooth, float smoothFactor = 0.1f) {
            Vector3 nextPosition = new Vector3(targetPosition.x, mainCamera.transform.position.y, targetPosition.z);
            //if the next position (after applying the offsets is not inside the panning limits)
            if (panLimit.enabled
                && (nextPosition.x < panLimit.minPosition.x
                || nextPosition.y < panLimit.minPosition.y
                || nextPosition.x > panLimit.maxPosition.x
                || nextPosition.y > panLimit.maxPosition.y)) {
                Debug.Log("RETURN DUDE" + currOffsetX + "    " + currOffsetZ);
                return;
            }
            mainCamera.transform.position = smooth ? Vector3.Lerp(mainCamera.transform.position, nextPosition, smoothFactor) : nextPosition;
        }
        ...
        private void Zoom() {
            ...
            targetHeight = Mathf.Lerp(maxHeight, minHeight, zoomValue);
            mainCamera.transform.position = Vector3.Lerp(mainCamera.transform.position,
                new Vector3(mainCamera.transform.position.x, targetHeight, mainCamera.transform.position.z), Time.deltaTime * zoomSpeed.value);
        }
    }
}
