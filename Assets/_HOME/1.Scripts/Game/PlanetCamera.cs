...
public class PlanetCamera : MonoBehaviour {
    ...
    void Awake() {
        if (gameObject.GetComponents<PlanetCamera>().Length > 1) {
    ...
    void SetShader() {
        ...
        string _shaderName = "StarBackroundPlanetSelect";
        ...
        customSkybox.GetComponent<Renderer>().sharedMaterial = new Material(Shader.Find("Custom/" + _shaderName));
        ...
        if (hasTintedStars) {
            customSkybox.GetComponent<Renderer>().sharedMaterial.SetColor("_StarColor", starsColor);
            customSkybox.GetComponent<Renderer>().sharedMaterial.SetFloat("_StarIntensity", starsIntensity);
        }
